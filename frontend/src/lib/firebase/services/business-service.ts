import { collection, doc, getDocs, setDoc, updateDoc, getDoc, query, where, orderBy } from 'firebase/firestore';
...
export class BusinessService {
  ...
  static async createBusiness(userId: string, userEmail: string, data: Omit<Business, 'id' | 'createdAt' | 'updatedAt' | 'userId' | 'primaryEmail'>): Promise<Business>;
  
  static async createBusiness(
    userIdOrData: string | { name: string; type: string; location?: string; currency?: string; businessNumber?: string; userId: string; primaryEmail: string; },
    userEmail?: string,
    legacyData?: Omit<Business, 'id' | 'createdAt' | 'updatedAt' | 'userId' | 'primaryEmail'>
  ): Promise<Business> {
    try {
      const businessRef = doc(collection(db, 'businesses'));
      
      let businessData: Omit<Business, 'createdAt' | 'updatedAt'> & { createdAt: Date; updatedAt: Date };
      ...
      try {
        const { LocalBusinessStorage } = await import('@/lib/utils/local-business-storage');
        LocalBusinessStorage.saveBusiness(businessData.primaryEmail, newBusiness);
        LocalBusinessStorage.setSelectedBusinessId(businessRef.id);
        console.log('Business saved to localStorage');
      } catch (error) {
  ...
  static async getBusinessesByUserEmail(email: string | null | undefined): Promise<Business[]> {
    if (!email) return [];
    
    try {
      // Use localStorage as fallback until Firestore permissions are fixed
      const { LocalBusinessStorage } = await import('@/lib/utils/local-business-storage');
      const localBusinesses = LocalBusinessStorage.getBusinesses(email);
      
      if (localBusinesses.length > 0) {
        console.log('Loading businesses from localStorage:', localBusinesses.length);
        return localBusinesses;
      }
      
      console.log('No businesses found in localStorage');
      return [];
    } catch (error: unknown) {
      console.error('Error getting businesses by user email:', error);
      return [];
    }
  }
  ...
  static async getBusinessByUserEmail(email: string | null | undefined): Promise<Business | null> {
    ...
    try {
      // Use localStorage as fallback until Firestore permissions are fixed
      const { LocalBusinessStorage } = await import('@/lib/utils/local-business-storage');
      const selectedBusiness = LocalBusinessStorage.getSelectedBusiness(email);
      
      if (selectedBusiness) {
        console.log('Loading selected business from localStorage:', selectedBusiness.name);
        return selectedBusiness;
      }
      
      // If no selected business, get the first one
      const businesses = LocalBusinessStorage.getBusinesses(email);
      if (businesses.length > 0) {
        const firstBusiness = businesses[0];
        LocalBusinessStorage.setSelectedBusinessId(firstBusiness.id);
  ...
  static async updateBusiness(businessId: string, data: Partial<Omit<Business, 'id' | 'createdAt' | '' | 'primaryEmail'>>) {
    const businessRef = doc(db, 'businesses', businessId);
